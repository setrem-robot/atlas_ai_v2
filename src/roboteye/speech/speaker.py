...
from roboteye.core.text import cleanForSpeech, truncate
...
def synthesizePolished(
    ...
    *,
    ...
    """
    cleaned = cleanForSpeech(text, language=language)
    if not cleaned:
        return
...
class Speaker:
    ...
    def __init__(
        ...
        self.polish = polish or AudioPolish()
        #: Idioma da voz, que decide como numeros e abreviacoes sao lidos.
        self.language = language

        self.queue: queue.Queue[Utterance | None] = queue.Queue()
        ...
        self.held: deque[Utterance | None] = deque()
        self.state = State()
        ...
        self.pronto: tuple[Utterance, list[SpeechChunk] | None] | None = None
        self.adiantando: threading.Thread | None = None
    ...
    def start(self) -> None:
        ...
        if self.thread is not None:
            return
        self.thread = threading.Thread(target=self.run, name="speaker", daemon=True)
        self.thread.start()
    ...
    def say(self, text: str) -> None:
        ...
        self.idle.clear()
        self.queue.put(Utterance(cleaned, generation))
    ...
    def sinalizar(self, chunks: tuple[SpeechChunk, ...]) -> None:
        ...
        with self.state.lock:
            generation = self.state.generation
        self.idle.clear()
        self.queue.put(Utterance("", generation, sinal=chunks))

    def endTurn(self) -> None:
        """Marca o fim de uma resposta: dispara `SpeechFinished` apos a ultima frase."""
        with self.state.lock:
            generation = self.state.generation
        self.queue.put(Utterance("", generation, endOfTurn=True))
    ...
    def waitUntilIdle(self, timeout: float | None = None) -> bool:
    ...
    # -- thread ------------------------------------------------------------
    def run(self) -> None:
        logger.debug("locutor iniciado (motor=%s, saida=%s)", self.engine.name, self.sink.name)
        while True:
            ...
            if preparada is None and self.adiantando is not None and self.adiantando.is_alive():
                self.adiantando.join()
                preparada = self.tomarPreparada()
            if preparada is not None:
                lote, chunks = preparada
                try:
                    self.speak(lote, chunks)
                    ...
                    # outra coisa.
                    self.anunciarSilencioSeAcabou()
                continue

            item = self.held.popleft() if self.held else self.queue.get()
            if item is None:
                break

            try:
                if not self.isCurrent(item.generation):
                    continue  # sobra de uma fala interrompida
                if item.endOfTurn:
                    if self.envelope is not None:
                        self.envelope.end()
                    self.bus.publish(SpeechFinished())
                    continue
                if item.sinal is not None:
                    self.tocarSinal(item.sinal)
                    continue
                self.speak(self.batch(item))
            except SpeechError as exc:
                logger.error("falha ao falar: %s", exc)
                self.bus.publish(ErrorOccurred(message=str(exc), source="speech"))
    ...
    def anunciarSilencioSeAcabou(self) -> None:
        ...
        """
        if self.queue.empty() and not self.held and self.pronto is None:
            self.setSpeaking(False)
            self.idle.set()

    def tomarPreparada(self) -> tuple[Utterance, list[SpeechChunk] | None] | None:
        """A fala adiantada, se houver uma valida esperando."""
        with self.state.lock:
            preparada, self.pronto = self.pronto, None

        if preparada is None:
            return None
        if not self.isCurrent(preparada[0].generation):
            # Alguem interrompeu entre preparar e falar.
            logger.debug("audio adiantado descartado: a fala foi interrompida")
            return None
        return preparada

    def adiantar(self, geracao: int) -> None:
        ...
        """
        if self.adiantando is not None and self.adiantando.is_alive():
            return
        with self.state.lock:
        ...
        try:
            proximo = self.held.popleft() if self.held else self.queue.get_nowait()
        except queue.Empty:
            return
        ...
        lote = self.batch(proximo, esperar=False)
        ...
        def trabalhar() -> None:
            ...
            with self.state.lock:
                self.pronto = (lote, chunks)

        self.adiantando = threading.Thread(target=trabalhar, name="speaker-adiantar", daemon=True)
        self.adiantando.start()

    def batch(self, first: Utterance, *, esperar: bool = True) -> Utterance:
        ...
        while length < MAX_BATCH_CHARS:
            try:
                item = self.queue.get(timeout=espera) if espera else self.queue.get_nowait()
            except queue.Empty:
                break
            espera = 0.0
    ...
    def speak(self, item: Utterance, pronto: list[SpeechChunk] | None = None) -> None:
        logger.info("falando: %s", truncate(item.text))
        self.setSpeaking(True)
        self.bus.publish(SpeechStarted(text=item.text))
        if self.envelope is not None:
            self.envelope.begin()
        ...
        for chunk in stream:
            ...
            self.adiantar(item.generation)
            self.sink.start(chunk.format)
            ...
            if self.envelope is not None:
                self.envelope.feed(chunk.audio, chunk.format)
            self.sink.write(chunk.audio)

    def tocarSinal(self, chunks: tuple[SpeechChunk, ...]) -> None:
