...
O preço está registrado no `whisperEars.py`, e é real — a mesma frase, no mesmo
microfone:
...
class VoskEars:
    ...
    def __init__(
        ...
        *,
        ...
        self.taxa = taxa
        self.model = None
        self.blocos: queue.Queue[bytes | None] = queue.Queue(maxsize=64)
    ...
    def retomar(self) -> None:
        if self.pausado.is_set():
            ...
            self.prepararOOuvido()
        self.pausado.clear()

    def prepararOOuvido(self) -> None:
        ...
        """
        mudo = self.paraInt16(np.zeros(BLOCO, dtype=np.float32))
        for _ in range(int(SILENCIO_DE_PARTIDA_S * TAXA / BLOCO)):
            with contextlib.suppress(queue.Full):
                self.blocos.put_nowait(mudo)
    ...
    def escutar(self) -> Iterator[Transcricao]:
        ...
        if self.model is None:
            raise HearingError(
                f"modelo de escuta ausente em {self.modelPath} "
    ...
    def umaCaptura(self, sd) -> Iterator[Transcricao]:
        """Uma sessão de captura, do `open` até o dispositivo parar."""
        from vosk import KaldiRecognizer

        filtro = PassaAlta(CORTE_GRAVES_HZ, self.taxa)

        def alimentar(entrada, quadros, tempo, status) -> None:
            if status:
            ...
            with contextlib.suppress(queue.Full):
                self.blocos.put_nowait(self.paraInt16(bloco))

        taxa, fator = negociarTaxa(sd, self.device)
        reconhecedor = KaldiRecognizer(self.model, self.taxa)
        with sd.InputStream(
            samplerate=taxa,
            ...
            yield from self.reconhecer(reconhecedor)

    @staticmethod
    def paraInt16(bloco: np.ndarray) -> bytes:
    ...
    def reconhecer(self, reconhecedor) -> Iterator[Transcricao]:
